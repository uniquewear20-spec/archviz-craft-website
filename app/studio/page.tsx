  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: "#080808", color: "#F0EBE3" }}>
      ...
      <div style={{ paddingTop: "100px" }}>
        <section className="grid grid-cols-1 lg:grid-cols-2" style={{ minHeight: "90vh" }}>
          ...
          <div className="flex flex-col justify-center px-8 md:px-14 lg:px-16 py-24">
            <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1.2, delay: 0.2 }}>
              <h1 className="font-serif font-extralight leading-[1.0]" style={{ fontSize: "clamp(2.8rem,5vw,4.2rem)", color: "#F0EBE3" }}>Wasim</h1>
              <h1 className="font-serif font-extralight leading-[1.0] italic" style={{ fontSize: "clamp(2.8rem,5vw,4.2rem)", color: "#A8885A" }}>Akram</h1>
            </motion.div>
            <motion.p classNt-sans font-light tracking-[0.12em] uppercase mt-5 mb-2" style={{ fontSize: "0.68rem", color: "#6B6560" }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1.0, delay: 0.45 }}>Senior 3D Visualizer &amp; Interior Designer</motion.p>
            <div className="mb-8 mt-6" style={{ height: "1px", backgroundColor: "#A8885A", width: "48px" }} />
            <motion.p className="font-sans font-light leading-[1.85] mb-10" style={{ fontSize: "clamp(0.85rem,1.1vw,0.95rem)", color: "#9A948E", maxWidth: "520px" }} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1.0, delay: 0.7 }}>Senior 3D Visualizer and Interior Designer with over 12 years of global experience, including 4+ years in the high-end UAE market. I specialize in crafting photorealistic architectural visualizations and cinematic animations for luxury villas, commercial spaces, and large-scale apartments.</motion.p>
            <motion.div className="flex flex-wrap gap-2" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1.0, delay: 0.85 }}>
              {skills.map((s) => (<span key={s} className="font-sans font-light px-3 py-1.5 tracking-[0.14em] uppercase" style={{ fontSize: "0.6rem", border: "1px solid #2A2520", color: "#6B6560" }}>{s}</span>))}
            </motion.div>
          </div>
        </section>
        <motion.section className="px-8 md:px-16 lg:px-24 py-24" style={{ borderTop: "1px solid #1C1916" }} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 1.0 }}>
          <div className="max-w-3xl mx-auto text-center">
            <span className="font-sans font-light tracking-[0.44em] uppercase block mb-8" style={{ fontSize: "0.6rem", color: "#A8885A" }}>Philosophy</span>
      ...
      <footer className="px-8 md:px-16 lg:px-24 py-8 flex items-center justify-between" style={{ borderTop: "1px solid #1C1916" }}>
        <a href="/" className="font-sans font-light tracking-[0.3em] uppercase" style={{ fontSize: "0.6rem", color: "#4A4540" }}>Back to Home</a>
